package io.github.grisstyl.magicscan.scan;
...
public class Scan implements ConfigurationSerializable, Describable {
    ...
    public boolean isOverriden(String ruleKey) {
                ...
                if (ruleType.equals(RuleType.SPELL)) {
                    ...
                    for (MagicSpell spell : newSpells) {
                        for (SpellRule rule : controller.getSpellRules()) {
                            if (!rule.shouldValidateRule() || this.isOverriden(rule.getKey())) continue;
    ...
    private void sendEntries(CommandSender sender, Map<? extends MagicType<?>, List<Violation>> map) {
        ...
        for (Map.Entry<? extends MagicType<?>, List<Violation>> entry : map.entrySet()) {
            ...
            }.runTaskLater(plugin, step * interval);
    ...
    public List<String> describe(CommandSender sender) {
        ...
        list.add("  &7Will be logged&8: &e" + BooleanFormat.format(getOptions().isLogToFile(), BooleanFormat.YES_NO));
        list.add("  &7Scan all rule types&8: &e" + BooleanFormat.format(getOptions().isScanAllRuleTypes(), BooleanFormat.YES_NO));
        list.add("  &7Scan hidden things&8: &e" + BooleanFormat.format(getOptions().isScanHidden(), BooleanFormat.YES_NO));
        list.add("  &7Visual&8: &e" + BooleanFormat.format(getOptions().isVisual(), BooleanFormat.YES_NO));
                ...
                if (rulesSection != null) {
                    for (String rulesKey : rulesSection.getKeys(false)) {
                        ...
                        if (section != null) {
                            ...
                            if (isOverriden(rulesKey)) {
                                bool = !bool;
                            }

                            list.add("    &8- &7" + rulesKey + "&8: " + BooleanFormat.format(bool, BooleanFormat.ENABLED_DISABLED));
                        }
                    }
                }
            }
