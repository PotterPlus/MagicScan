...
import io.github.grisstyl.magicscan.file.MagicDefFile;
import io.github.grisstyl.magicscan.file.MessagesFile;
import io.github.grisstyl.magicscan.file.RulesFile;
...
import io.github.grisstyl.magicscan.rule.spell.*;
...
public class MagicScanController {
    ...
    protected MagicScanController(MagicScanPlugin plugin) {
        ...
        if (magicPlugin instanceof MagicAPI) {
            ...
            PluginLogger.atInfo()
                    .with("[MagicScan] Successfully integrated with Magic API.")
                    .print();
        } else {
            PluginLogger.atSevere()
                    .with("[MagicScan] Failed to integrate with Magic API! MagicScan has been disabled! Please install Magic to use MagicScan.")
                    .print();
    ...
    public void refresh() {
        ...
        if (config.getFileConfiguration().getBoolean("pastebin_integration", false)) {
            ...
            if (devKey == null) {
                PluginLogger.atWarn()
                        .with("[MagicScan] Failed to set up Pastebin integration with invalid dev key!")
                        .print();
            } else {
                this.pastebin = pastebinFactory.createPastebin(devKey);

                PluginLogger.atInfo()
                        .with("[MagicScan] Pastebin integration successfully set up.")
                        .print();
            }
            ...
            if (config.isVerbose()) {
                PluginLogger.atInfo()
                        .with("[MagicScan] Pastebin integration is disabled.")
                        .print();
            }
        }
    ...
    protected void refreshActions() {
        ...
        try {
            ...
            JsonNode n = spellParametersNode.path("spell_parameters");
